...
def test_power():
    ...
    ox_mass, h_mass, cell_mass = power_plant_mass(average_power=shuttle_power, mission_length_days=8, safety_factor=1)
    # Mass of 2 power cells and contents of 3 oxygen and hydrogen tanks
    ground_truth_mass = 2 * lb_to_kg(255) + 3 * lb_to_kg(781 + 92)

    assert np.isclose(ox_mass + h_mass + cell_mass, ground_truth_mass, rtol=1e-3)
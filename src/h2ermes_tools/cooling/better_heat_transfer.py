...
def run_simulation(
    ...
    if thermal_conductivity is None:
        thermal_conductivity = SS310.thermal_conductivity
    ...
    if isinstance(incident_heat_flux, np.ndarray):
        # Expecting shape (N, 2): time, value
        times = incident_heat_flux[:, 0]
        values = incident_heat_flux[:, 1]
        def incident_flux_func(t):
            return np.interp(t, times, values)
    elif callable(incident_heat_flux):
        incident_flux_func = incident_heat_flux
    else:
        def incident_flux_func(t):
            return incident_heat_flux
    ...
    for step in range(num_time_steps):
        ...
        if callable(specific_heat):
            ...
            cp_nodes = specific_heat * np.ones_like(temperature)
        if callable(thermal_conductivity):
            k_nodes = thermal_conductivity(temperature)
        else:
            k_nodes = thermal_conductivity * np.ones_like(temperature)
        # Internal nodes
        for idx in range(1, num_nodes - 1):
            ...
            temperature_next[idx] = temperature[idx] + (
                time_step * k_avg / (density * cp_nodes[idx] * node_spacing**2)
        ...
        cpN = cp_nodes[-1]
        net_heat_flux_last = (
            kN * (temperature[num_nodes - 2] - temperature[num_nodes - 1])
            / node_spacing
            - cooling_heat_flux
        )
        ...
        temperature_next[num_nodes - 1] = temperature[
            ...
            density * cpN * control_volume_last
        )
...
if __name__ == "__main__":
    ...
    wall_thickness = 20e-3
    num_nodes = 3
    # Example: time-dependent incident heat flux (ramps up then down)
    incident_heat_flux_profile = np.array([
        [0, 100_000],
        [450, 666_000],
        [900, 0],
    ])
    # To test, set incident_heat_flux to the array above:
    ...
    if do_animation:
        animate_profiles(
            ...
            time_step,
            node_spacing,
            interval=2,  # Animation frame interval in milliseconds

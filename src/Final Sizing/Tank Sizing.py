...
def tank_overall_dimensions():
    ...
    R = ((520 + 99) * 3 * np.tan(np.radians(phi))/ (np.pi* (1 - ratio_radius)* (1 + ratio_radius + ratio_radius**2)))*(1 / 3)
    r = R * ratio_radius
...
def calculate_tank_thickness(wet_mass, propellant_pressure, fuel_mass, tank_length, R, r, phi, E, strength, thrust, gamma=0.65):
    ...
    t_press = (propellant_pressure/6894.76 * R)/(np.cos(phi) * (strength * 0.85 - propellant_pressure/6894.76)) #Pressure vessel design
    while t < max_thickness:
        ...
        M_cr_bend = (0.33+0.1)*(2*np.pi * E * r * t**2* np.cos(phi)**2)/(3*(1-0.33**2)) + np.pi*r**3*propellant_pressure/2
        P_cr_axial = (0.33+0.1)*(2*np.pi * E * t**2* np.cos(phi)**2)/(3*(1-0.33**2)) + np.pi*r**2*propellant_pressure
        p_cr = (0.92*E*0.75)/((tank_length/average_radius)*(average_radius/t)**(5/2))
...
print("----------------------------------------------------------------------------------------------")
tank_length_LH2, R_LH2, r_LH2 = calculate_tank_length_LH2("LH2", LH2_volume, radius_ratio, phi, r_LOX)

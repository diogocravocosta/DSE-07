...
def calculate_tank_thickness(wet_mass, propellant_pressure, fuel_mass, tank_length, tank_diameter, E, strength, thrust, gamma=0.65):
    ...
    while t < max_thickness:
        ...
        p_cr = 0.926*E*np.sqrt(gamma)/(((tank_diameter/2)/t)**(5/2) * (tank_length/(tank_diameter/2)))
        ...
        interaction = safety_factor * (sigma_axial/sigma_cr_axial + sigma_bend/sigma_cr_bend + propellant_pressure/p_cr)
        if interaction <= 1.0:
            break
